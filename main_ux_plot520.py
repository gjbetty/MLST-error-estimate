...
class CasualPINN():
    def __init__(self,layers,M,x_star,y_star,usol,usol_ux,usol_ut,nt,nx,tol):
        ...
        T,X = np.meshgrid(self.t_r,self.x_r)

        self.X_r = torch.tensor(np.concatenate([X.flatten()[:,None],T.flatten()[:,None]],axis = 1),requires_grad=True).float().to(device)
        ...
        # IC 2048
        self.x_ic = torch.tensor(x_star).float().reshape(-1,1)
        self.t_ic = -1*torch.ones_like(self.x_ic)

        self.X_ic = torch.concat([self.x_ic,self.t_ic],dim = 1).to(device)
        self.Y_ic = torch.tensor(y_star).float().to(device)
        self.X_ic.requires_grad = True


        Nbc = 256
        x_boundary_left = torch.cat((2*torch.rand([Nbc, 1])-1, -10*torch.full([Nbc, 1], 1)), dim=1)
        x_boundary_right = torch.cat((2*torch.rand([Nbc, 1])-1, 10*torch.ones([Nbc, 1])), dim=1)
        ...
        self.u_bc = torch.zeros(512, 1).float().to(device)
        self.u_0 = torch.zeros(512, 1).float().to(device)
    ...
    def train(self,model,model_save_path,pre_model_save_path,nIter = 10000):
        ...
        for it in pbar:
            ...
            loss = self.loss(it,x_save,y_save)
            ...
            if it>0 and it % tim == 0:  # 1000
                loss_value = self.loss(it,x_save,y_save).detach().cpu().numpy()
                ...
                loss_res_value = self.loss_res(x_save,y_save).detach().cpu().numpy()
    ...
    def predict(self,X):
        ...
        self.NN.eval()
        u = self.net_u(x,t).detach().cpu().numpy()
        return u
...
# u_cosh = 0.5 * (np.exp(0.5*np.sqrt(1.5)*(5*X_ref-0.1*1.5*T_ref-5)) + np.exp(-0.5*np.sqrt(1.5)*(5*X_ref-0.1*1.5*T_ref-5)))
# u_sech = 1/u_cosh
usol = 3 * 3 / np.cosh(np.sqrt(3) / 2 * (X_ref - 3 * T_ref)) ** 2
usol_ic = 3 * 3 / np.cosh(np.sqrt(3) / 2 * (X_ic - 3 * T_ic)) ** 2
...
pre_model_save_path = 'C:/Users/18090/Desktop/4th-revise/kdv_save_ux_plot_520.pt'  #kdv_save_ux_plot.pt
model_save_path = 'C:/Users/18090/Desktop/4th-revise/kdv_save_ux_plot_520-test.pt'

model = CasualPINN(layers,M,x_ic,state0,usol,usol_ux,usol_ut,n_t,n_x,tol)
...
u_pred = model.predict(X_p).reshape(T.shape)
...
plt.subplot(1, 3, 1)
plt.plot(x, usol[:,0], color='blue',label='exact')
plt.plot(x, u_pred[:,0], '--', color='red',label='predict')
plt.xlabel('$x$',fontsize=20,labelpad=1)
plt.ylabel('$u(t, x)$',fontsize=20,labelpad=1)
...
plt.subplot(1, 3, 2)
plt.plot(x, usol[:,50], color='blue',label='exact')
plt.plot(x, u_pred[:,50], '--', color='red',label='predict')
plt.xlabel('$x$',fontsize=20,labelpad=1)
plt.ylabel('$u(t, x)$',fontsize=20,labelpad=1)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
...
plt.subplot(1, 3, 3)
plt.plot(x, usol[:,-1], color='blue',label='exact')
plt.plot(x, u_pred[:,-1], '--', color='red',label='predict')
plt.xlabel('$x$',fontsize=20,labelpad=1)
plt.ylabel('$u(t, x)$',fontsize=20,labelpad=1)
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
...
plt.legend(fontsize=14)
plt.tight_layout()
plt.savefig(fig_save_path + 'bigfinal_result_legend.pdf', dpi = 300)
plt.close()
